...
async function captureComponentScreenshot(browser, component, breakpoint, breakpointLabel, serverPort) {
  ...
  try {
    ...
    const parallaxOffsets = {
      ...
      projects: 1.3, // Projects offset={1} + factor={2}, capture middle at offset 1.5
    ...
    console.log(
      `📍 Scrolled using ${actualScrollResult.method}: ${actualScrollResult.scrollPos}px (target: ${actualScrollResult.targetPos}px)`
    )
...
async function captureScreenshots(targetComponent = null) {
  ...
  try {
    ...
    if (CI_MODE) {
      console.log(
        `⚡ Quality: ${SCREENSHOT_QUALITY}, Animation wait: ${ANIMATION_WAIT_TIME}ms, Format: ${SCREENSHOT_TYPE.toUpperCase()}`
      )
    } else {
      console.log(
        `📸 Format: ${SCREENSHOT_TYPE.toUpperCase()}, Quality: ${SCREENSHOT_QUALITY}, Animation wait: ${ANIMATION_WAIT_TIME}ms`
      )
    }
    ...
    const componentsToCapture = targetComponent
      ? validComponents.includes(targetComponent)
        ? [targetComponent]
        : []
      : validComponents
    ...
    if (targetComponent && !validComponents.includes(targetComponent)) {
      console.warn(
        `⚠️ Component '${targetComponent}' does not have a defined scroll position. Valid components are: ${validComponents.join(", ")}`
      )
    }
