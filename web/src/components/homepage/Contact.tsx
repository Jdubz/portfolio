...
const Contact = ({ offset, factor = 1 }: { offset: number; factor?: number }) => (
  ...
  >
    ...
    <Divider speed={0.1} offset={offset} factor={factor} className="iconCanvas">
      ...
      <UpDown>
        <Svg icon="wrench" width={80} stroke color="icon_brightest" left="28%" top="10%" />
        <Svg icon="plug" width={88} stroke color="icon_purple" left="82%" top="90%" />
      </UpDown>
      <UpDownWide>
        {/* Top cluster - floating to surface */}
        <Svg icon="hexa" width={64} color="icon_teal" left="88%" top="5%" />
        <Svg icon="arrowUp" hiddenMobile width={72} color="icon_orange" left="48%" top="4%" />
        <Svg icon="battery" width={56} color="icon_green" left="5%" top="12%" />

        {/* Bottom cluster - rising bubbles */}
        <Svg icon="cross" width={72} color="icon_pink" left="15%" top="88%" />
        <Svg icon="robotic-arm" width={80} color="icon_yellow" left="52%" top="92%" />
      </UpDownWide>
      {/* Mid-section - sparser, smaller bubbles for depth */}
      <Svg icon="json" width={32} stroke color="icon_brightest" left="92%" top="50%" />
    </Divider>
  </section>
)
