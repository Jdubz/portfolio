...
**Layer 1: Smarter AI Prompts** (Quick Win)
- Strengthen prompts with specific constraints:
  - Resume: Limit to 3-4 most relevant experience entries, max 4 bullets per entry
  - Cover Letter: Strict 2-3 body paragraphs, max 150 words per paragraph
  - Add explicit word count targets for each section
  - Emphasize brevity and conciseness as a requirement

**Layer 2: Content Filtering** (Recommended)
- Pre-filter experience data before sending to AI:
  - Automatically select top 3-4 most recent/relevant experiences
  - Trim or summarize blurbs that exceed reasonable length
  - Reduces AI cost AND ensures reasonable document length

**Layer 3: Post-Generation Validation** (Safety Net)
- Add validation after AI generation:
  - Count words/characters in generated content
  - Log warning if content exceeds targets
  - Optional: Implement automatic truncation with continuation indicator

**Implementation Priority:**
1. High: Attribution footer (30 min)
2. High: Update AI prompts with stricter constraints (1-2 hours)
3. Medium: Add experience entry selection logic (2-3 hours)
4. Medium: Add word count validation (1-2 hours)
5. Low: Auto-truncation fallback (2-3 hours)

**Complexity:** ~6-10 hours total
